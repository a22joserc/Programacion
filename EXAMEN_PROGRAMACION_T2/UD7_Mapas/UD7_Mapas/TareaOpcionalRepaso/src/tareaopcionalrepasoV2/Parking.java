...
public class Parking {
	...
	public boolean entradaVehiculo(Vehiculo vehiculo, LocalDateTime fechaEntrada) { // throws Exception {
		...
		vehiculo.getListaRegistrosParkings().add(new Estacionamiento(fechaEntrada));
	...
	public boolean salidaVehiculo(Vehiculo vehiculo, LocalDateTime fechaSalida) { // throws Exception {
		...
		vehiculoParking.setFacturacion();
	...
	public double getFacturacionParking(LocalDateTime fechaReferencia) {
		double facturacionParking = 0;
		Iterator<Vehiculo> iterator = this.getRegistroVehiculos().iterator();
		while (iterator.hasNext()) {
			Vehiculo vehiculo = iterator.next();
			Iterator<Estacionamiento> iteratorEstacionamiento =vehiculo.getListaRegistrosParkings().iterator();
			while (iteratorEstacionamiento.hasNext()) {
				...
				if (estacionamiento.getFechaEntrada().isAfter(fechaReferencia.minusHours(24)) && estacionamiento.getFechaSalida() != null) {
					facturacionParking += vehiculo.getFacturacion();
				}
			}
		}
	...
	public void getFicheroResumenPrintWriter(LocalDateTime fechaReferencia) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				printWriter.println("Facturación vehículo: " + vehiculo.getFacturacion());
				printWriter.println("***********************************************************");
			}
			printWriter.println("Facturación últimas 24 horas: " + this.getFacturacionParking(fechaReferencia) + " €");
			printWriter.println("***********************************************************");
			printWriter.close();
