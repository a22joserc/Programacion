...
public class GestionPerson {
	...
	public void getRegistroPosicion(JdbcRowSet rowSet, int posicion) throws SQLException{
	    ...
	    if (rowSet.absolute(posicion)) {
	    	...
	    	System.out.println("Registro posicion seleccionada (" + posicion +"): " + personPos.toString());
	    }
	    else {
	    	System.out.println("La posición indicada no existe la tabla de registros");
	    }
	    
	    if (rowSet.next()) {
	    	Person personNext = new Person(rowSet.getInt(1), rowSet.getString(2), rowSet.getString(3), rowSet.getInt(4));
	    	System.out.println("Registro posicion siguiente a la seleccionada (" + (posicion + 1) + "): " + personNext.toString());
	    }
	    else {
	    	System.out.println("La posición "  + (posicion + 1) + " no existe la tabla de registros");
	    }
	    ...
	    if (rowSet.previous()) {
	    	...
	    	System.out.println("Registro posicion previa a la seleccionada (" + (posicion - 1) + "): " + personPrevious.toString());
	    }
	    else {
	    	System.out.println("La posición "  + (posicion - 1) + " no existe la tabla de registros");
	    }
	...
	public static void main(String[] args) {
		...
		try(JdbcRowSet rowSet= gestionPerson.getJdbcRowSet()){
			...
			gestionPerson.getRegistroPosicion(rowSet, 3);
